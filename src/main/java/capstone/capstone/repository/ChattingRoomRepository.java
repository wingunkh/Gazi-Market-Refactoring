...
public interface ChattingRoomRepository extends JpaRepository<ChattingRoom, Integer> {
    ...
    @Query(value="select * from ChattingRoom where (ChattingRoom.guest_member = :guest_num OR ChattingRoom.host_member = :guest_num) AND ChattingRoom.host_member != 71 order by ChattingRoom.cht_room_num", nativeQuery = true)
    List<ChattingRoom> findAllId(@Param("guest_num") int guest_num);

    @Query(value="select * from ChattingRoom chr where chr.guest_member = :guest_num AND chr.post_num = :post_num AND chr.host_member != 71", nativeQuery = true)
