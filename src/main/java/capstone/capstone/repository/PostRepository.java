...
public interface PostRepository extends JpaRepository<Post, Integer> {
    ...
    @Query(value = "SELECT * FROM Post p WHERE p.model_name IN (SELECT m.model_name FROM Model m WHERE m.category_name = :category_name) AND p.status = '판매중' ORDER BY p.written_date desc", nativeQuery = true)
    List<Post> getPostByCategory(@Param("category_name")String category_name);

    @Query(value = "SELECT * FROM Post p WHERE p.model_name = :model_name AND p.status = '판매중' ORDER BY p.written_date desc", nativeQuery = true)
    List<Post> getPostByModel(@Param("model_name")String model_name);
